...
def sync_libraries_from_config() -> None:
    ...
    try:
        for entry in libraries_cfg:
            ...
            # A configured library can keep its ID while its path changes.
            # Resolve the stable ID first to avoid inserting a duplicate
            # primary key, then fall back to path matching for legacy configs.
